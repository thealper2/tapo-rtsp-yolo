...
while True:
    ...
    # Initialize a flag to check if any object is detected within the AOI
    object_detected_in_aoi = False
    ...
    for result in results[0].boxes.data.tolist():
        ...
        if score > 0.5:
            ...
            # Check if the detected object is within the AOI
            if (x1 > aoi[0] and y1 > aoi[1] and x2 < aoi[2] and y2 < aoi[3]):
                object_detected_in_aoi = True
                cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 2)  # Red rectangle
                cv2.putText(frame, f"{class_name} {score:.2f}", (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    ...
    cv2.rectangle(frame, (aoi[0], aoi[1]), (aoi[2], aoi[3]), (0, 255, 0) if not object_detected_in_aoi else (0, 0, 255), 2)
